...
public class DifferentDataUtil {
     ...
     * @return
     */
    public static List<CompanyPackagingDataInfo> addPackagingDataToLocal(List<CompanyPackagingDataInfo> netList, List<CompanyPackagingDataInfo> localList) {
        List<CompanyPackagingDataInfo> mCompanyInfoAddList = new ArrayList<>();//差异数据
        List<String> mNetList = new ArrayList<>();
        List<String> mLocalList = new ArrayList<>();
        for (CompanyPackagingDataInfo companyInfo : netList) {
            mNetList.add(companyInfo.getId());
        }
        if (!ListUtils.isEmpty(localList)) {
            for (CompanyPackagingDataInfo companyInfo : localList) {
                mLocalList.add(companyInfo.getId());
            }
            for (String string : mNetList) {
                if (!mLocalList.contains(string)) {
     ...
     * @return
     */
    public static List<CompanyExpenditureInfo> addExpenditureDataToLocal(List<CompanyExpenditureInfo> netList, List<CompanyExpenditureInfo> localList) {
        List<CompanyExpenditureInfo> mCompanyInfoAddList = new ArrayList<>();//差异数据
        List<String> mNetList = new ArrayList<>();
        List<String> mLocalList = new ArrayList<>();
        for (CompanyExpenditureInfo companyInfo : netList) {
            mNetList.add(companyInfo.getId());
        }
        if (!ListUtils.isEmpty(localList)) {
            for (CompanyExpenditureInfo companyInfo : localList) {
                mLocalList.add(companyInfo.getId());
            }
            for (String string : mNetList) {
                if (!mLocalList.contains(string)) {
     ...
     * @return
     */
    public static List<PayRecordInfo> addPayRecordDataToLocal(List<PayRecordInfo> netList, List<PayRecordInfo> localList) {
        List<PayRecordInfo> mCompanyInfoAddList = new ArrayList<>();//差异数据
        List<String> mNetList = new ArrayList<>();
        List<String> mLocalList = new ArrayList<>();
        for (PayRecordInfo companyInfo : netList) {
            mNetList.add(companyInfo.getId());
        }
        if (!ListUtils.isEmpty(localList)) {
            for (PayRecordInfo companyInfo : localList) {
                mLocalList.add(companyInfo.getId());
            }
            for (String string : mNetList) {
                if (!mLocalList.contains(string)) {
     ...
     * @return
     */
    public static List<CompanyExpenditureInfoB> addExpenditureBDataToLocal(List<CompanyExpenditureInfoB> netList, List<CompanyExpenditureInfoB> localList) {
        List<CompanyExpenditureInfoB> mCompanyInfoAddList = new ArrayList<>();//差异数据
        List<String> mNetList = new ArrayList<>();
        List<String> mLocalList = new ArrayList<>();
        for (CompanyExpenditureInfoB companyInfo : netList) {
            mNetList.add(companyInfo.getId());
        }
        if (!ListUtils.isEmpty(localList)) {
            for (CompanyExpenditureInfoB companyInfo : localList) {
                mLocalList.add(companyInfo.getId());
            }
            for (String string : mNetList) {
                if (!mLocalList.contains(string)) {
     ...
     * @return
     */
    public static List<RoleInfoBean> addRoleDataToLocal(List<RoleInfoBean> netList, List<RoleInfoBean> localList) {
        List<RoleInfoBean> mCompanyInfoAddList = new ArrayList<>();//差异数据
        List<String> mNetList = new ArrayList<>();
        List<String> mLocalList = new ArrayList<>();
        for (RoleInfoBean companyInfo : netList) {
            mNetList.add(companyInfo.getId());
        }
        if (!ListUtils.isEmpty(localList)) {
            for (RoleInfoBean companyInfo : localList) {
                mLocalList.add(companyInfo.getId());
            }
            for (String string : mNetList) {
                if (!mLocalList.contains(string)) {
     ...
     * @return
     */
    public static List<CompanyUserInfoBean> addUserDataToLocal(List<CompanyUserInfoBean> netList, List<CompanyUserInfoBean> localList) {
        List<CompanyUserInfoBean> mCompanyInfoAddList = new ArrayList<>();//差异数据
        List<String> mNetList = new ArrayList<>();
        List<String> mLocalList = new ArrayList<>();
        for (CompanyUserInfoBean companyInfo : netList) {
            mNetList.add(companyInfo.getId());
        }
        if (!ListUtils.isEmpty(localList)) {
            for (CompanyUserInfoBean companyInfo : localList) {
                mLocalList.add(companyInfo.getId());
            }
            for (String string : mNetList) {
                if (!mLocalList.contains(string)) {
