...
public class CompanyDeliveryOrderManagementActivity extends BaseRefreshMvpActivity<CompanyDeliveryOrderPresenter> implements CompanyDeliveryOrderContract.View {
    ...
    public void pullDownRefresh(int page) {
        if (mRoleCode.equals("companyRoot")) {
            mPresenter.getCompanyDeliveryOrderList(page, rows, mCompanyId, mUserId, et_quotation_name_a.getText().toString().trim(), tv_start_time.getText().toString().trim(), tv_end_time.getText().toString().trim());
        } else {
            mPresenter.getCompanyDeliveryOrderList(page, rows, mCompanyId, String.valueOf(UserInfoManager.getUserLoginInfo(this).getId()), et_quotation_name_b.getText().toString().trim(), tv_start_time.getText().toString().trim(), tv_end_time.getText().toString().trim());
        }
    }
