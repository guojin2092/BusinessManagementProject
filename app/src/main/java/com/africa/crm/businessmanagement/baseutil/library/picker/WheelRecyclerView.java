package com.africa.crm.businessmanagement.baseutil.library.picker;
...
public class WheelRecyclerView extends RecyclerView {
    ...
    private final int DEFAULT_SELECT_TEXT_COLOR = getResources().getColor(R.color.app_main_color);

    private final int DEFAULT_UNSELECT_TEXT_COLOR = getResources().getColor(R.color.black);
    ...
    private final int DEFAULT_DIVIVER_COLOR = Color.parseColor("#F6F6F6");
    ...
    private List<TimeBean> mDatas; //数据
    ...
    public void setData(List<TimeBean> datas) {
        if (datas == null) {
            return;
        }
        mDatas.clear();
    ...
    private class WheelAdapter extends Adapter<WheelAdapter.WheelHolder> {
        ...
        public void onBindViewHolder(WheelHolder holder, int position) {
            ...
            if (position < mOffset || position > mDatas.size() + mOffset - 1) {
                ...
                holder.name.setText(mDatas.get(position - mOffset).time);
            }
        }
    ...
    public interface OnSelectListener {
        void onSelect(int position, TimeBean data);
    }
