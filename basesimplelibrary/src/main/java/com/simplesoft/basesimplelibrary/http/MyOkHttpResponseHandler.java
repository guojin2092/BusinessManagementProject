...
import com.simplesoft.basesimplelibrary.utils.MyLog;
...
/*        MyLog.e(context, "methodName(" + methodName + ")" + new String(responseBody));
        ...
        // 如果已经登录返回成功
                /*String json = new String(responseBody);*/
        try {
            String getResponse = response.body().string().toString();
            object = gson.fromJson(getResponse, Class.forName(classname.getName()));
            Logger.e(methodName + "-->" + getResponse);
            Logger.json(getResponse);
            if (object != null) {
                synchronized (this) {
                    if (!dealLogin) {
                        showLoginOverTint((BaseSimpleBean) object);
